...
\begin{figure}
    ...
    Different symbols show different substrates, where Eq.~(\ref{eq:theta_const}) is shown by blue bullets (\textcolor{black}{$\bullet$}) and Eq.~(\ref{eq:theta_band}) by orange stars (\textcolor{black}{$\star$}).
...
This is indicated in by the $n_{\max} = 1$ data Fig.~\ref{fig:max_drops}, where the blue bullets (\textcolor{jlblue}{$\bullet$}) show data from the banded pattern. 
In those simulations the ring-rivulet has not ruptured at the end of the simulation time.
However, $\Delta h$ measurements show growth in most cases.
...
Similarly to the uniform substrate and in agreement with pervious work~\cite{gonzalezStabilityLiquidRing2013} we sometimes see an evolution as shown in Fig.~\ref{fig:ThreeDToOneD}.
Thus, having $n = n_{\max}$ for early time scales but at rupture the system chooses $n < n_{\max}$.

\subsubsection{Linear wettability gradients}\label{subsubsec:linwettgrad}
So far we have shown that our numerical experiments can reproduce known results and that upon using a patterned substrate we are able to remove the coalescence mode.
