...
A lower contact angle yields a smaller rivulet, both in height and width for the same $r_0$ value, which in turn affects $\psi_0$.
...
We also saw that Eq.~(\ref{eq:maxDrops}) holds true for $\psi_0 \leq 0.25$, for larger $\psi_0$ collapse is more likely.
...
TODO: Add text set context with Edwards et al.~\cite{edwardsControllingBreakupToroidal2021}.
