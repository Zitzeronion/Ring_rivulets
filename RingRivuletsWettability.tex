...
\end{split}
\end{equation}
where the discrete lattice velocities $\mathbf{c}_l$ run from $0$ to $8$, for our two-dimensional scheme.
This equation is build on the idea that there is a local collision step in which the distribution functions are relaxed towards their equilibrium $f_l^{(eq)}(\mathbf{x},t)$ with relaxation rate $\omega = \Delta t/\tau$ and a non-local collision step, where the $f_l(\mathbf{x},t)$ are streamed to their nearest neighbours.
...
We use the standard $D2Q9$ (athermal) values, therefore $c_s = 1/\sqrt{3}$, the lattice velocities $\mathbf{c}^{(l)}$~\cite{krugerLatticeBoltzmannMethod2017}   
