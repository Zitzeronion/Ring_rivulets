...
\begin{figure}
    ...
    \caption{Thickness difference $\Delta h$ normalized by the height of a spherical cap droplet that contains all liquid $H_D$ along the curve at radius $\xi(h_{\max})$ over time normalized by $\tau_{m}$~\cite{wuBreakupPatternedNanoscale2010} for $\theta(\xi) = \theta_0$ (Eq.~(\ref{eq:theta_const})). 
...
\end{equation}
where $a$ is a numerical constant which is set to $a = 0.0379$ according to Ref.~\cite{wuBreakupPatternedNanoscale2010} and $A$ is half of the area of the cross-section of the rivulet as shown in Fig.~\ref{fig:ringschema}(c).
We like to mention that this growth-rate differs from ref~\cite{wuBreakupPatternedNanoscale2010} by a factor of $\theta^{-3/2}$.
From Eq.~(\ref{eq:growth-sigma}) we can derive a timescale $\tau_m$ by simply inverting the growth rate  
\begin{equation}\label{eq:tau_m}
    \tau_{m} = \frac{1}{\sigma_m} = \frac{6\mu\sin(\theta)\sqrt{A}}{\gamma a \theta^{3/2}\sqrt{\theta - \sin(2\theta)/2}}.      
\end{equation}
In Fig.~\ref{fig:first_growth} we show $\Delta h$ data for Eq.~(\ref{eq:theta_const}) normalized by their respective $H_D$ as the different symbols.
On x-axis we display the time normalized with $\tau_m$.  
We find a very good collapse of our data with this rescaling and as a guide to eye we add a black dashed line that is proportional to $e^{\sigma_m t}$.
Interestingly the full symbols follow the same exponential growth, Eq.~(\ref{eq:growth-sigma}), the empty symbols on the other hand do not show a pronounced growth, except the open triangles ($\triangle$, $\psi_0 = 0.173$). 


and is shown as dashed line in Fig.~\ref{fig:first_growth}.
The filled symbols are numerical experiments that lead to break up of the rivulet using Eq.~(\ref{eq:theta_const}), while the empty symbols display the collapse into a single droplet.
It turns out that the behavior is not only sensitive to the geometry, $\psi_0$ but also to $\theta$ and therefore we introduce the time scale



The $\Delta h$ values along $\xi(h_{\max})$ of the rivulet allow us to clearly distinguish between initial conditions that lead to breakups and droplet fragmentation, as such following the dashed line in Fig.~\ref{fig:first_growth}, and a collapse into a single droplet as shown by the open symbols in Fig.~\ref{fig:first_growth}.
The open triangles ($\triangle$, $\psi_0 = 0.173$) actually mark an edge case in which $\Delta h$ grows with a similar slope as the filled symbols, however the time scale on which the instability grows is too long as compared to the collapse time scale. 
