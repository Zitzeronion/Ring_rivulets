...
# ╔═╡ d807fdd1-a0ed-47ea-931d-5757d348ab48
...
begin
	xvals = collect(0.05:0.01:0.23)
	uniplot = plot(xlabel=L"\psi_0", 
	ylabel=L"n_d", 
	xlims=(0.03, 1.1), 
	ylims=(0.9, 100), 
	xscale=:log10, 
	...
	scatter!(dataUni10.psi0, dataUni10.ndrops, label="θ₀ = 10°", mc=palette(:default)[1], ms=10, ma=0.7, m=:circ)
	scatter!(dataUni20.psi0, dataUni20.ndrops, label="θ₀ = 20°", mc=palette(:default)[2], ms=10, ma=0.7, m=:rect)
	scatter!(dataUni30.psi0, dataUni30.ndrops, label="θ₀ = 30°", mc=palette(:default)[3], ms=10, ma=0.7, m=:star5)
	scatter!(dataUni40.psi0, dataUni40.ndrops, label="θ₀ = 40°", mc=palette(:default)[4], ms=10, ma=0.7, m=:diamond)

	plot!(xvals, π ./ (2 .* xvals), label="π/(2ψ₀)", l = (:black, :dash, 2))
end
...
begin
	xvals2 = collect(0.035:0.01:0.8)
	banplot = plot(xlabel=L"\psi_0", 
	ylabel=L"n_d", 
	xlims=(0.03, 1.1), 
	ylims=(0.9, 100), 
	xscale=:log10, 
	...
	dataBan40 = subset(dataBa, :theta => s -> s .== 40, :ndrops => d -> d .> 0)
	...
	scatter!(dataBan40.psi0, dataBan40.ndrops, label="θₐ = 40°", mc=palette(:default)[4], ms=10, ma=0.7, m=:diamond)
	
	plot!(xvals2, π ./ (2 .* xvals2), label="π/(2ψ₀)", l = (:black, :dash, 2))
end

# ╔═╡ cc0b1c29-7cfc-4d02-b381-5a9f61a4e65c
begin
	dataBan40t = subset(dataBa, :theta => s -> s .== 40)
	scatter(dataBan40t.psi0, dataBan40t.ndrops)
end
...
# ╔═╡ 6dc842c3-3289-4305-a36c-be55add0af2e
function simpleT(angle;γ=0.01, μ=1/6)
	return 1/(γ*0.0379*(angle)^3/(6μ*sin(angle)))
end
...
begin
	...
	# plot!(times_now ./ Atscale[1], (0.025 .* exp.(0.1 .* times_now ./ Atscale[1]) .+ 0.00) ./ finalH[1], label="∝ exp(σₘt)", l = (3,  :dashdot, :darkgreen))
	#plot!(subdata2.time[1:end] ./ T0, (0.001 .* exp.(0.000008 .* subdata2.time[1:end]) .+ 0.14) ./ H0, 
		#label="",
		# l = (2,  :dashdot, :black))
...
begin
	growthsDrop = plot(xlabel = L"t/\tau_m", ylabel = L"\Delta h / (R_0 - R_f)",
		# xaxis=:log10,
		yaxis=:log10,
		grid = false,
		legendfontsize = 11,
	...
	incondsDrop = [(200, 80, 20, π/9, :rt), (150, 80, 20, π/9, :ut), (180, 80, 20, π/9, :circ), (150, 40, 30, 2π/9, :hex), (180, 80, 40, 2π/9, :dt), (150, 40, 30, π/6, :rect), (180, 40, 30, π/6, :diamond)]
	for ics in incondsDrop
		iC = initial_data[(initial_data.R0 .== ics[1]) .& (initial_data.rr0 .== ics[2]) .& (initial_data.angle .== ics[3]), :]
...
begin
	symdict = Dict(10=>:circ, 20=>:rect, 30=>:star, 40=>:diamond, 50=>:utriangle)
	mar = Symbol[]
	col = Int64[]
	for i in breakup.theta
		push!(mar, symdict[i])
		push!(col, i // 10)
	end
	breakup.markers = mar
	...
	br10 = subset(breakup, :theta => a -> a .== 10)
	br20 = subset(breakup, :theta => a -> a .== 20)
	br30 = subset(breakup, :theta => a -> a .== 30)
	br40 = subset(breakup, :theta => a -> a .== 40)
end
...
begin
	...
	scatter!(br10.psi0, 
		br10.breakupT ./ (1/6 .* (br10.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 10°",
		m = (8, :circ, 0.75),
		color = palette(:default)[1]
	)
	scatter!(br20.psi0, 
		br20.breakupT ./ (1/6 .* (br20.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 20°",
		m = (8, :rect, 0.75),
		color = palette(:default)[2]
	)
	scatter!(br30.psi0, 
		br30.breakupT ./ (1/6 .* (br30.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 30°",
		m = (8, :star, 0.75),
		color = palette(:default)[3]
	)
	scatter!(br40.psi0, 
		br40.breakupT ./ (1/6 .* (br40.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 40°",
		m = (8, :diamond, 0.75),
...
# ╔═╡ 750ec973-ce48-4ff0-94bb-9adbcc2d2a14
savefig(bPlot, "../assets/breakup_tc.pdf")

# ╔═╡ d529d32d-7481-4ce2-8b83-5ac75d5fc9ef
begin
	mar2 = Symbol[]
	col2 = Int64[]
	for i in collapses.theta
		push!(mar2, symdict[i])
		push!(col2, i // 10)
	end
	collapses.markers = mar2
	collapses.colors = col2
	
	c20 = subset(collapses, :theta => a -> a .== 20)
	c30 = subset(collapses, :theta => a -> a .== 30)
	c40 = subset(collapses, :theta => a -> a .== 40)
end

# ╔═╡ 5dc20a23-7d6f-45c7-b121-fabac658cb16
begin
	tau_C = (1/6 .* (collapses.rr0) ./ 0.01)
	someax = collect(0.16:0.01:0.6)
	cPlot = plot(xlabel=L"\psi_0", 
		ylabel = L"\tau_c\theta_0^2 / t_c",
		yaxis=:log10,
		xaxis=:log10,
		grid = false,
		legendfontsize = 11,
	...
	scatter!(c20.psi0, 
		c20.collapsT .* (deg2rad.(c20.theta)).^2 ./ (1/6 .* (c20.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 20°",
		m = (8, :rect, 0.75),
		color = palette(:default)[2]
	)
	scatter!(c30.psi0, 
		c30.collapsT .* (deg2rad.(c30.theta)).^2 ./ (1/6 .* (c30.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 30°",
		m = (8, :star, 0.75),
		color = palette(:default)[3]
	)
	scatter!(c40.psi0, 
		c40.collapsT .* (deg2rad.(c40.theta)).^2 ./ (1/6 .* (c40.rr0) ./ 0.01), #breakup.tauM, 
		label="θ₀ = 40°",
		m = (8, :diamond, 0.75),
		color = palette(:default)[4]
	)
	plot!(someax,25 .* someax.^(-2), c=:black, l=(2,:dash), label="")
end

# ╔═╡ 63956ef7-bb53-44e5-93f6-3b74e74199f4
savefig(cPlot, "../assets/psi_0_collapse.pdf")

# ╔═╡ 9e72c5bd-d56a-4dad-ae99-248d5736af4a
tbreakband = subset(timeScaleDF2corr, :breakupT => s -> s .> 1)

# ╔═╡ 29f37c46-b450-4637-92f3-1d5518d4c17a
begin
	dth = Int64[]
	mband = Symbol[]
	colsband = Int64[]
	for i in tbreakband.theta
		push!(dth, 60 - i)
		push!(mband, symdict[60 - i])
		push!(colsband, (60 - i) // 10)
	end
	tbreakband.markers = mband
	tbreakband.deltatheta = dth
	tbreakband.colors = colsband
	
	bb20 = subset(tbreakband, :deltatheta => a -> a .== 20)
	bb30 = subset(tbreakband, :deltatheta => a -> a .== 30)
	bb40 = subset(tbreakband, :deltatheta => a -> a .== 40)
	bb50 = subset(tbreakband, :deltatheta => a -> a .== 50)
end

# ╔═╡ 79e1860f-0044-4651-aa10-db77376ca33a
begin
	tau_Cb = (1/6 .* (tbreakband.rr0) ./ 0.01)
	someotherax = collect(0.037:0.001:0.18)
	bPlotBand = plot(xlabel=L"\psi_0", 
		ylabel = L"\tau_b / t_c",
		yaxis=:log10,
		xaxis=:log10,
		grid = false,
		...
		ylims = (500, 10000),
		xlims = (0.03, 0.205),
		xticks = ([0.03, 0.04, 0.05, 0.07, 0.1, 0.2, 0.3],["0.03", "0.04", "0.05", "0.07", "0.1", "0.2", "0.3"])
	)
	scatter!(bb50.psi0, 
		bb50.breakupT ./ (1/6 .* (bb50.rr0) ./ 0.01), #breakup.tauM, 
		label="θₐ = 10°",
		m = (8, :circ, 0.75),
		color = palette(:default)[1]
	)
	scatter!(bb40.psi0, 
		bb40.breakupT ./ (1/6 .* (bb40.rr0) ./ 0.01), #breakup.tauM, 
		label="θₐ = 20°",
		m = (8, :rect, 0.75),
		color = palette(:default)[2]
	)
	scatter!(bb30.psi0, 
		bb30.breakupT ./ (1/6 .* (bb30.rr0) ./ 0.01), #breakup.tauM, 
		label="θₐ = 30°",
		m = (8, :star, 0.75),
		color = palette(:default)[3]
	)
	scatter!(bb20.psi0, 
		bb20.breakupT ./ (1/6 .* (bb20.rr0) ./ 0.01), #breakup.tauM, 
		label="θₐ = 40°",
		# zcolor = breakup.theta,
		m = (8, :diamond, 0.75),
		color = palette(:default)[4]
	)
	plot!(someotherax,50000 .* someotherax.^(1.0) .- 200, c=:black, l=(2, :dash), label="")
end

# ╔═╡ bd72ee4e-603b-4930-9557-b6c94ff0ccf7
savefig(bPlotBand, "../assets/breakup_band_hmm.pdf")
...
begin
	...
	Roft = plot(xlabel = L"t/t_c", ylabel = L"R(t)/R_0",
		# yaxis=:log10,
		# xaxis=:log10,
		grid = false,
		legendfontsize = 10,
	...
	plot!(plain.time ./ (1/6 .* (plain.rr0) ./ 0.01), plain.R_t ./ plain.R0, label="θ₀ = 40°", l=(2.5, :solid))
	# plot!(band.time ./ tt, band.R_t ./ ll, label="Eq.(8), θ = 40°", l = (2.5, :dash))
	plot!(g1040.time ./ (1/6 .* (g1040.rr0) ./ 0.01), g1040.R_t ./ g1040.R0, label="θₐ = 10°", l = (2.5, :dash))
	plot!(g2040.time ./ (1/6 .* (g2040.rr0) ./ 0.01), g2040.R_t ./ g2040.R0, label="θₐ = 20°", l = (2.5, :dashdot))
	plot!(g3040.time ./ (1/6 .* (g3040.rr0) ./ 0.01), g3040.R_t ./ g3040.R0, label="θₐ = 30°", l = (2.5, :dot))
end
...
begin
	...
	Roftpos = plot(xlabel = L"t/t_c", ylabel = L"R(t)/R_0",
		# yaxis=:log10,
		# xaxis=:log10,
		grid = false,
		legendfontsize = 10,
	...
	plot!(plain20.time ./ (1/6 .* (plain20.rr0) ./ 0.01), plain20.R_t ./ plain20.R0, label="θ₀ = 40°", l=(2.5, :solid))
	# plot!(band20.time ./ (1/6 .* (g1040.rr0) ./ 0.01), band20.R_t ./ ll2, label="Eq.(8)", l = (2.5, :dash))
	plot!(g4020.time ./ (1/6 .* (g4020.rr0) ./ 0.01), g4020.R_t ./ g4020.R0, label="θₐ = 40°", l = (2.5, :dash))
	plot!(g3020.time ./ (1/6 .* (g3020.rr0) ./ 0.01), g3020.R_t ./ g3020.R0, label="θₐ = 30°", l = (2.5, :dashdot))
end

# ╔═╡ 23b6e6e6-4a81-48bb-897f-5586c95d7b48
savefig(Roftpos, "../assets/radius_time_gradient_positive.pdf")

# ╔═╡ 0d887016-c613-4c50-b555-79bc51dab64e
CSV.write("../data/ring_all_data.txt", growthDF, delim=' ')
...
manifest_format = "2.0"
project_hash = "4dcbfa2e34c5f161f43179ad0f090f6304108ec2"
